...
public:
   ...
   FT_GEStatus Cmd_CSketch(int16_t x, int16_t y, uint16_t w, uint16_t h,
      uint32_t Ptr, uint16_t Format, uint16_t Freq);//api to send csketch command
};
...
FT_Status FT801Impl<FT_Trans>::Init(uint8_t ResType, uint16_t options1 = 0,
   bool flipped = false) {
   return (FT_GC < FT_Trans > ::Init(ResType, options1, flipped));
}

/* Api to program the output display wrt display parameters */
template<class FT_Trans>
FT_Status FT801Impl<FT_Trans>::Init(uint16_t hperiod, uint16_t vperiod,
   ...
   FT_Status Status;
   Status = FT_GC < FT_Trans
      > ::Init(hperiod, vperiod, hfrontporch, hbackporch, hpulsewidth,
         vfrontporch, vbackporch, vpulsewidth, polarity, swizzle, fps,
         options1);
...
template<class FT_Trans>
void FT801Impl<FT_Trans>::GetVersion(uint8_t &Major, uint8_t &Minor,
   uint8_t &Build) {
...
template<class FT_Trans>
FT_GEStatus FT801Impl<FT_Trans>::Cmd_CSketch(int16_t x, int16_t y, uint16_t w,
   ...
   Status = FT_GC < FT_Trans
      > ::WriteCmd(((Freq & 0xFFFFL) << 16) | (Format & 0xFFFFL));
